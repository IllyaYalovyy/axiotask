...
**Q: What happens to a "sync needs attention" state across an app restart?**
It is deliberately **not** persisted. A *permanent* (non-transient, non-auth) sync failure — store corruption, a schema mismatch, a deserialization bug — is stuck: retrying is pointless until something changes, so the scheduler backs the idle cadence off (`attention_streak` drives an exponential backoff) and the UI raises a persistent "needs attention" indicator (Properties, and the main-window sidebar — #136). Both the streak counter and the `needs_attention` flag are ephemeral runtime state: they start at 0/false on every launch, and the **first** post-restart sync re-derives the correct value. This is intentional: (1) `needs_attention` is a symptom of a *live* failure, not a stored fact — if the restart itself cleared the cause we must not show a stale warning, and if the cause is genuinely permanent the next run re-flags it immediately; (2) a deliberate relaunch is a legitimate reason to retry at the base cadence rather than resuming an already-stretched backoff; (3) persisting it would be a store-schema change (forbidden pre-1.0) that could strand a stale indicator after a fix. The distinct dead-session state (`needs_reauth`) is likewise re-derived from the stored tokens on launch, not from a persisted flag.

**Q: How are parent/subtask due dates kept consistent — and why is sync excluded?**
Ruled 2026-08-01 (#164). If a parent has an **explicit** due date, no child may hold an explicit date **before** it. Enforced only at the moment of a local edit in our UI, with the editor's intent winning: setting the parent's date pulls any earlier children **up** to it; setting a child's date earlier than the parent's pulls the **parent down** to match. A parent with no explicit date makes the rule inert, and rows without explicit dates never participate. Every date-entry path (detail panel, picker, quick-add NL, reschedule, bulk) goes through one shared command-layer primitive; each cascade is a single undo unit and announces itself in a toast. **Deliberately no repair on pull**: a violation arriving from another client (the Google UI honors no such rule) is *displayed as-is* and re-normalizes on the next local touch of either date. Repairing on pull would either diverge local from server under a matching etag (a P6 violation) or push corrections that overwrite a human's explicit choice made in another client — acceptable for structural repairs (D7's third-level flatten), wrong for a preference-shaped rule. An alternative — forbidding parent dates outright — was rejected as unenforceable for the same reason: remote clients can always create one. Do not add pull-side normalization; do not treat a displayed remote violation as a bug.
