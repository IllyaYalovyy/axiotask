...
// a drop emits reorder steps counting ONLY same-list siblings (cross-list smart-
// view cards are skipped), and a same-level (top-level-only) flat list. The
// touch-hold timing lives in T8.1; here the desktop drag-handle path is covered.
...
void main() {
  ...
  group('reorderStep (step-count, same-list-only)', () {
    final singleList = [
      row('A', 'a'),
      row('B', 'b'),
      row('C', 'c'),
    ...
    test('a downward move counts every row it lands past', () {
      // A (0) → landing index 2 (past B and C).
      expect(reorderStep(singleList, 0, 2), (direction: 'down', steps: 2));
    });

    test('an upward move counts every row it lands before', () {
      // D (3) → landing index 1, crossing C and B → two single-step swaps up.
      expect(reorderStep(singleList, 3, 1), (direction: 'up', steps: 2));
    });

    test('a drop in place is a no-op', () {
      expect(reorderStep(singleList, 1, 1), isNull);
    });
    ...
    test('cross-list cards are NOT counted as reorder siblings', () {
      ...
      // A (0) → landing index 2 crosses X (other list, skipped) and B (same).
      expect(reorderStep(crossList, 0, 2), (direction: 'down', steps: 1));
    });
    ...
    test('a move crossing only other-list cards is a no-op', () {
      ...
      expect(reorderStep(crossList, 0, 2), isNull);
    });
  });
  ...
  testWidgets('dragging a row down calls reorder_task once per step crossed', (
    tester,
  ) async {
    ...
    // One same-list step down → exactly one reorder_task('A','down').
    expect(fake.reordered, ['A:down']);
  });
}
