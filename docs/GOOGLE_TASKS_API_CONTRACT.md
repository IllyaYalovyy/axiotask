...
| DPoP | Google's installed-app documentation describes DPoP for sender-constraining refresh tokens: ES256 with a P-256 public JWK, an authorization-code-derived `jti`, unique refresh `jti` values, and server nonce handling. It also states that the resulting access token remains a bearer token. The 2026-08-15 Linux probe successfully exchanged and refreshed with the bound key; omitting the proof was rejected with HTTP 400 `invalid_request`. | **Officially documented; observed by a controlled probe** | Linux persists the DPoP private key atomically with its refresh token and never substitutes another key silently. A missing proof is known to fail, but the probe must not require Google to return a different undocumented error code. |
