...
| Task delete | P6/P4 recorded repeated delete success in earlier cases; S34A later observed 412 when replaying with the original pre-delete ETag while read-back retained the tombstone. DELETE through a stale source-list path after a concurrent move remains incompletely evidenced. | Read back/resolve the task ID first after uncertainty, then repeat only from current evidence to enforce authoritative deletion. Never confirm from old-list absence, a stale conditional replay, or an unverified stale-path response. |
