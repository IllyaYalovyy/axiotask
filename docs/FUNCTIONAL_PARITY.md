...
|---|---|---|---|---|---|---|---|---|
| `PAR-AUTH-001` | Connect Google account | `app:commands.rs` auth commands; `design:RFC-001-auth-oauth-pkce.md` | **Redesign** — replace Tauri flows and plaintext desktop persistence with accepted platform adapters. | Linux browser PKCE/DPoP and future native Android authorization expose one typed account state; connection alone never implies healthy sync. | [Authentication boundary](ARCHITECTURE.md#authentication-boundary), [ADR 0003](adr/0003-authentication.md) | U,C,W,I,G,D,V | Linux is implemented: S05/S19B provide secure authorization, refresh, and Reauthorize; first-run Connect verifies the Google subject before durably creating/selecting its account partition, reopens the normal runtime, and starts full verification. Missing build configuration and every failed/cancelled/storage outcome remain visible with no premature account or Google work. Android authorization is not implemented or device-verified. | Deterministic authorization/HTTP/sync suites plus `test/app/first_run_authorization_test.dart` and `test/features/tasks/{tasks_view_model,adaptive_shell}_test.dart` pass. The complete clean-tree default Linux acceptance gate, configured debug/release builds, and the opt-in GNOME Secret Service probe pass. The live Google contract still requires fresh browser consent for the dedicated account before it can produce provider evidence. Android D/V evidence is not run. |
