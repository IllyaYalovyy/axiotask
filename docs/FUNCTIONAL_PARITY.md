...
Implementation and verification are platform-specific: Linux evidence does not
qualify Android, and an available opt-in command is not recorded as executed
human or real-account evidence until that run actually occurs.
...
|---|---|---|---|---|---|---|---|---|
| `PAR-AUTH-001` | Connect Google account | `app:commands.rs` auth commands; `design:RFC-001-auth-oauth-pkce.md` | **Redesign** — replace Tauri flows and plaintext desktop persistence with accepted platform adapters. | Linux browser PKCE/DPoP and future native Android authorization expose one typed account state; connection alone never implies healthy sync. | [Authentication boundary](ARCHITECTURE.md#authentication-boundary), [ADR 0003](adr/0003-authentication.md) | U,C,W,I,G,D,V | Linux is implemented: S05/S19B provide secure authorization, refresh, and Reauthorize; first-run Connect verifies the Google subject before durably creating/selecting its account partition, reopens the normal runtime, and starts full verification. Missing or invalid XDG runtime configuration and every failed/cancelled/storage outcome remain visible with no premature account or Google work. Android authorization is not implemented or device-verified. | Deterministic authorization/HTTP/sync suites plus `test/app/first_run_authorization_test.dart` and `test/features/tasks/{tasks_view_model,adaptive_shell}_test.dart` pass. The complete clean-tree default Linux acceptance gate, debug/release builds, opt-in GNOME Secret Service probe, and S34A live Google contract through the shipped authorization/Tasks adapters pass. Android D/V evidence is not run. |
...
| `PAR-UX-001` | Theme and appearance Settings | `ui:theme.js`; `ui:theme.css`; `ui-test:ThemeContrast.test.js` | **Retain; storage redesign**. | System/light/dark and standard/compact density are device-only typed preferences with system/default-safe behavior and accessible contrast. | [Persistence](ARCHITECTURE.md#persistence) | U,W,I,V | S22A typed namespaced device storage, malformed-value recovery, diagnostics, restart, and write-failure behavior complete; S32B projects shared visual tokens; the Linux header now opens a native Settings page before or after account connection and applies changes immediately. | Device/native preference suites; `test/features/settings/{settings_view_model,settings_view}_test.dart`; Settings navigation in `test/features/tasks/adaptive_shell_test.dart`; pre-Connect access in `test/app/first_run_authorization_test.dart`; light/dark and density visual-token evidence. Android device V remains unverified. |
| `PAR-UX-002` | Onboarding | `ui:App.svelte` onboarding | **Redesign**. | Explain Connect, truthful sync status, offline continuity, quick add, and where recovery lives; dismissal is device-only. | [Trust before decoration](UX.md#trust-before-decoration) | W,I,V | S22A typed namespaced dismissal storage/default/restart complete; S32B adds an accessible adaptive first-run presentation without a false sync claim; fresh Linux installs present onboarding and keep Settings reachable before Connect. A failed dismissal write is diagnosed but never blocks connection. | Onboarding ViewModel/widget/keyboard/text-scale tests, light/dark goldens and inspected synthetic Linux captures, plus `test/app/first_run_authorization_test.dart`. Android device V remains unverified. |

## Safety, recovery, links, development-only behavior, and non-goals

| ID | Capability | Rust behavioral evidence | Decision and intentional difference | Intended Flutter behavior | Domain / UX specification | Required evidence | Implementation | Verification |
|---|---|---|---|---|---|---|---|---|
| `PAR-DATA-001` | Backup/export | `core:export.rs`; `ui-test:Export.test.js` | **Retain; redesign format boundary** — export user state, not Rust sync/storage rows. | Versioned account snapshot includes supported projected data and explicit privacy warning. | [Backup/export contract](#backupexport-and-restoreimport) | U,P,W,I,S,V | S30A adds the strict bounded `axiotask.accountBackup` v1 document, selected-account projected repository, acknowledged-offline/list/hierarchy/order preservation, native Fedora save picker, and persistent private-data warning/result UI | Codec/bounds/privacy unit tests; real-SQLite account/offline repository tests; file cancel/failure tests; ViewModel/widget and isolated Linux application integration; privacy/quality gates; inspected 1280×720 warning/result captures |
| `PAR-DATA-002` | Restore/import | `app:state.rs::restore_backup`; `ui-test:Import.test.js` | **Retain; redesign safety semantics**. | After successful target sync, recreate absent records; matching existing identity wins; no content matching, overwrite, or deletion. | [Backup/import contract](#backupexport-and-restoreimport) | U,C,P,W,I,S,G,V | S30B strict bounded open/decode, fresh-sync refusal, same-subject identity planning, atomic provisional projection/desired-state/manifest restore, same-partition retry idempotence, and explicit duplicate limits complete | Codec/planner/file/store/ViewModel/widget/Linux integration and create-engine partial-success suites; dedicated real-Google smoke; inspected preview/result Fedora captures |
| `PAR-DATA-003` | Reset Local Data | `app:commands.rs::fresh_sync` clears synced rows before pull. | **Redesign as explicit destructive product recovery** — pending work is intentionally discarded too. | Reset selected account partition after warning/confirmation, preserve authorization/device preferences, then rebuild from Google with truthful health. | [Reset contract](#refresh-recovery-and-reset-local-data) | U,P,W,I,S,V | S31 adds selected-account preview/confirmation, coordinator cancellation and drain, one cascading rollback-safe partition transaction, full rebuild, truthful failed-empty outcome, and fail-closed development isolation | Domain/store/coordinator/ViewModel/widget/golden tests; isolated Linux application integration; inspected warning/rebuilt/failed Fedora captures |
| `PAR-LINK-001` | Open every task in Google Tasks | `ui-test:OpenInGoogle.test.js`; task `webViewLink` | **Retain as an all-task escape hatch**. | Every task shows a clearly labeled action that opens a validated Google task link; unavailable/invalid links remain visibly explained. It may expose any Google UI capability unavailable through the API, including recurrence. | [Google Tasks UI and links](UX.md#google-tasks-ui-and-links) | U,C,W,I,G,D,V | S32A task-detail action, strict Google-link validation, disabled/missing state, and external launcher failure state complete | Domain/adapter/widget coverage is complete. S34A passed through the shipped adapters and verified live `webViewLink` presence/shape. Actual Linux navigation is on the `verify_linux_acceptance.sh --human` checklist; Android device navigation remains unverified. |
| `PAR-LINK-002` | Open user-authored web links | `ui:TaskDetail.svelte::extractUrls`; `ui-test:UrlDetection.test.js` | **Retain separately**. | Parse only supported `http`/`https` URLs from plain text; require safe launcher outcome and never shell-execute. | [Network and API data](SECURITY.md#network-and-api-data) | U,C,W,I,D,V | S32A strict plain-text extraction, distinct action/semantics, launcher port, and visible launch failure complete | Domain/adapter/widget coverage is complete. Actual Linux navigation is on the human acceptance checklist; Android device navigation remains unverified. |
| `PAR-LINK-003` | Edit recurrence | No public recurrence fields in the Tasks discovery contract. | **Drop as unsupported** — never simulate it locally. | The all-task Google UI action can expose recurrence when Google provides a link; recurring configuration is not cached or claimed. | [API limitations](GOOGLE_TASKS_API_CONTRACT.md#deletion-hierarchy-dates-completion-and-recurrence) | U,W,I,G,V | S32A deliberately has no local recurrence model or editor | S34A verified the live ordinary-task link contract. Actual Linux navigation and recurrence escape-hatch review remain human evidence; Android remains unverified. |
| `PAR-RECOVERY-001` | Database-open recovery | Rust schema wipe/export behavior in `core:store/mod.rs` | **Redesign** — no automatic destructive replacement. | Preserve unreadable files, stop remote work, show Retry Open plus safe diagnostics; Reset remains a separate explicit action. | [Persistence recovery](SYNC_SPEC.md#database-failures) | U,P,W,I,V | S21B preserves unreadable main/WAL/SHM files and supplies repeated Retry Open; S31 makes destructive selected-partition reset a separately confirmed healthy-storage action | `test/app/database_recovery_test.dart`; file/recovery suites; Reset widget/golden/Linux integration; inspected recovery and reset captures |
...
| `PAR-DEV-003` | Multiple isolated instances/prefixes | `core:config.rs` `AXIOTASK_PREFIX` | **Drop from product**. | Tests inject isolated paths, storage/credential/diagnostic namespaces, transports, and accounts; users do not manage instance prefixes. The Linux runner keeps its compiled native application ID and uses `G_APPLICATION_NON_UNIQUE`, so concurrent process support is real but data isolation never depends on a per-instance GTK ID. | [Isolation rules](TESTING.md#isolation-rules) | U,P,I | Synthetic composition and acceptance smoke use non-Google transports plus distinct database/namespaces; the acceptance bundle also receives temporary XDG roots in a detached worktree. | Composition/isolation/first-run suites and the fake-command acceptance orchestration test; no product-facing instance feature. |
...
an XDG-isolated synthetic bundle smoke, debug/release production builds, and a
...
The clean-tree noninteractive gate and autonomous review of every current
synthetic Linux screenshot have passed. **HUMAN task 56** now contains only
hands-on product acceptance, any unavoidable Google browser consent, and the
decision to authorize Android work. Agents own command execution, live-probe
continuation after consent, failure remediation, and routine screenshot/golden
review. No approval is implied by the noninteractive gate. Multi-account UI
remains deferred while account partitioning is present from schema version 1.
Android authorization, adaptive device behavior, physical-device evidence, and
final Android visual/release qualification remain explicitly unverified and
sequenced after Linux approval. Optional read-only tooling remains isolated test
